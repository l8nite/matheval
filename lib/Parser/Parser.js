...
Parser.prototype.infix2postfix = function(tokens, stack, outputQ) {
    this.args = -1; // running argument counter for functions
    ...
    for (var i = 0; i < tokens.length; ++i) {
        ...
        if (this.args === 0 && token.isnt('RightParen')) {
            this.args = 1; // after this, only commas increase args
        }
...
_i2p_Handlers['LeftParen'] = function(outputQ, stack, token) {
    stack.push(new Token(Types.Number, this.args));
    this.args = 0;
    stack.push(token);
};
...
_i2p_Handlers['RightParen'] = function(outputQ, stack, token) {
    ...
    var args = this.args;
    this.args = stack.pop().value;

    // pop function token onto output queue
    if (stack.peek().is('Function')) {
        // later: handle left-associative function args?
        outputQ.push(new Token(Types.Number, args));
